...
def fetch_admin_user_details_view(request):
    try:
        user = request.user
        if user.has_permission_to_view_admins_data(user):
...
def fetch_seller_user_details_view(request):
    try:
        user = request.user
        if user.has_permission_to_view_sellers_data(user):
            serializer = SellerUserSerializer(user)
            return Response({"user": serializer.data}, status=status.HTTP_200_OK)
        else:
            return Response(
                {"detail": "Permission denied."}, status=status.HTTP_403_FORBIDDEN
...
def fetch_user_details_view(request):
    try:
        user = request.user
        if user.has_permission_to_view(user):
